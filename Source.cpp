...
int main(int argc, const char **argv) {
	...
	for (int k = 4; k < argc; k++) {
		...
		if (cv::cuda::getCudaEnabledDeviceCount() == 0) // Unless CUDA device is not working properly
		{
		...
		cv::cuda::printShortCudaDeviceInfo(cv::cuda::getDevice()); // Showing CUDA device

		cv::cuda::GpuMat gpuimg; // Creating GPU matrix
		cv::cuda::GpuMat gpuimg1;

		gpuimg.upload(img); // Uploading image from CPU to GPU
		gpuimg1.upload(img1);
		...
		CascadeClassifier gpucascade;
		gpucascade.load(name_of_classifier/*name of classifier*/);

		TickMeter tm;
		tm.start();

			//if (cascade.load(name_of_classifier/*name of classifier*/) /*loading the cascade classifier*/)  
			if (gpucascade.load(name_of_classifier/*name of classifier*/) /*loading the cascade classifier*/) {
			...
			gpucascade.detectMultiScale(gpuimg, drops, 1.05, 3, 0, Size(5, 5), Size(570, 570));
			...
			for (int i = 0; i < drops.size(); i++) {
				...
				circle(gpuimg1, center, radius, Scalar(255, 0, 0), 2);

			}

			namedWindow(windowName, WINDOW_AUTOSIZE);
			setMouseCallback("Supervising", (CvMouseCallback)supervisedCircles, &img1);
			//setMouseCallback("Supervising", (CvMouseCallback)supervisedCircles, &Gpuimg1);
			...
			while (keepProcessing) {
				...
				if (key == 'n') {
					...
					//imwrite(tempPath, Gpuimg1);
					keepProcessing = false;
				}

				imshow(windowName, img1);
				//imshow(windowName, Gpuimg1);
