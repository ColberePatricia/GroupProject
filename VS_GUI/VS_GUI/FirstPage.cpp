...
// The individual unit tests of the functions used in the program
void testClassifier() {
	// We test the classifier with the Haar5.xml cascade classifier
	name_of_classifier = "C:/Users/Patricia/Desktop/Haar5.xml";
	if (cascade.load(name_of_classifier)) {
		cout << "The Haar5 classifier has been loaded\n";
		// Should we also test detectMultiScale?
...
void testCalculateCenter() {
	...
	P3.x = -1;
	P3.y = -4;
	calculateCenter(P1, P2, P3);
	assert(S.x == 9);
	assert(S.y == -8);
	assert(radiusS == 10);
}

void testSupervisedCircles() {
	// Complicated to test
}

// The test of the whole program
void testProgram() {
	cout << "Test of the program\n";
	testClassifier();
	testCalculateCenter();
	testSupervisedCircles();
